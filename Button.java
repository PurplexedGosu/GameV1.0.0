...
public class Button extends Actor
{
    ...
    public void act() 
    {
        ...
        if(delay>20&&Greenfoot.mouseClicked(this)&&type==1&&ninja.getPoints()>0)
        {
            delay = 0;
            ninja.setMeleeDamage(ninja.getMeleeDamage()+1);
            ninja.setPoints(ninja.getPoints()-1);
        }
        if(delay>20&&Greenfoot.mouseClicked(this)&&type==2&&ninja.getPoints()>0)
        {
            delay = 0;
            ninja.setRangeDamage(ninja.getRangeDamage()+1);
            ninja.setPoints(ninja.getPoints()-1);
        }
        if(delay>20&&Greenfoot.mouseClicked(this)&&type==3&&ninja.getPoints()>0)
        {
            delay = 0;
